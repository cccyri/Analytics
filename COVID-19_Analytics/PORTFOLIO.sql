...
select Location, Population, date, MAX(CAST(CovidDeaths.total_cases AS FLOAT)) as HighestInfectionCount, MAX((CAST(CovidDeaths.total_cases AS FLOAT)/NULLIF(CAST(CovidDeaths.Population AS FLOAT),0))*100) as PercentageInfectedMax
from PortfolioDataBase..CovidDeaths
